...
def compute_reward(quad_state, quad_vel, collision_info, current_dist):
    thresh_dist = 7
    beta = 1
    dist = current_dist

    z = -10
    pts = [np.array([-.55265, -31.9786, -19.0225])]
    ...
    if collision_info.has_collided:
        ...
        dist = min(dist, np.linalg.norm(np.cross((quad_pt - pts[0])
...
def current_distance(current_dist):
   thresh_dist = 7
    beta = 1
    dist = current_dist
    ...
    dist = min(dist, np.linalg.norm(np.cross((quad_pt - pts[i])
...
while True:
    ...
    reward = compute_reward(quad_state, quad_vel, collision_info, current_dist)
    done = isDone(reward)
